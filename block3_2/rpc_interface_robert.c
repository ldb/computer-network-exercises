...
struct element *do_rpc(char *out_buffer, int size) {
	...
	if (incoming_header.get == 1) {
		...
		element->key = malloc(incoming_header.key_length);
		element->value = malloc(incoming_header.value_length);
...
struct element *get(char *key, int keylen) {
	...
	element_t *e = do_rpc(outbuffer, final_size);
